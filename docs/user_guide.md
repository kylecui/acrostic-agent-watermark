...
>
> **适用场景边界**（七轮独立外部验证判定）：内部审计/防内部泄露 **推荐**（长文档 ≥1200 字检出 90–100%）；
> 中低强度攻击溯源 **可用**；**法律取证级不适用**（算法边界）。短稀疏文本自动分级
> `reliability: low/medium`，照常嵌入但不承诺稳定溯源。详见 [README「适用场景与能力边界」](../README.md)。
