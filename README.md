...
**Agent 级别的数字水印** —— 用"藏头诗"的思路，在 agent 输出的 token 层面嵌入**可解码还原用户身份**的水印。
...
# 多用户区分 + 攻击鲁棒性
python examples/02_multiuser_robustness.py

# 内容寻址锚点：抗插入/删除编辑（v0.3）
python examples/03_edit_robustness.py
...
✅ **v0.4 句子边界感知指纹 + 中文支持已实现（63 项单元测试全通过）**：
...
│   └── design.md              # 设计文档：架构、算法、威胁模型、v0.2-v0.4
├── src/
│   └── aawm/
│       ├── __init__.py
│       ├── keys.py            # 密钥派生（HKDF-SHA256，支持位置/指纹标签）
│       ├── coding.py          # 信道编码：CRC-8 / 重复码 / 交织重复码 / 汉明(7,4)
│       ├── anchor.py          # 锚点位置选择（PRF 排序，v0.2 遗留）
│       ├── transforms.py      # 谓词 + KeyedLetterMap 密钥符号映射（v0.4 支持任意字母表）
│       ├── embedder.py        # v0.2 嵌入器 + 稳定化词典（v0.4 扩充 + 中文）
│       ├── decoder.py         # v0.2 解码器
│       ├── content.py         # v0.3 内容寻址锚点 + v0.4 句子边界感知
│       ├── synonym_data.py    # v0.4 词典数据（英文扩充 + 中文同义组）
│       ├── zh.py              # v0.4 语言适配器（EnAdapter / ZhAdapter）
│       ├── verifier.py        # 检测封装（CRC 判定 + z-test 统计）
│       └── stats.py           # 统计检验（z-score, p-value）
├── tests/                     # 63 项单元测试（v0.2 32 + v0.3 15 + v0.4 16）
├── examples/
│   ├── 01_minimal_embed.py    # 嵌入并解码用户 ID
│   ├── 02_multiuser_robustness.py  # 多用户区分 + 攻击鲁棒性
│   └── 03_edit_robustness.py  # 内容寻址 vs 位置索引的编辑攻击对比
├── experiments/
│   ├── exp_edit_attacks.py    # 编辑攻击系统评测 + paraphrase 评测（可复现）
