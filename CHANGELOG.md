...
第 10 轮外部验证问题修复（GitHub issue #23）：soft 归因的"真值缺席"守门。

- **#23 soft 路径真值缺席误归因**：`embed()` 不要求 UID 已注册，嵌入
  UID 不在注册库时，默认 soft 路径会以高置信把水印归因到无关的注册
  用户（实测干净往返 4/5 错怪；hard 路径 0/5 正确弃权）。根因：soft
  是最近邻分类器，`match_margin` 拒绝的是候选间"歧义"，拒绝不了
  "真值缺席时候选集体皆错"
- **修复（得分充分性守门）**：硬解码位 = sign(z)，故信号质量上限
  s_max = Σ|z|（= 硬解码值的理论 soft 得分）。胜出候选得分须 ≥
  `match_score_floor`（默认 0.5）× s_max，否则判定"注册库全体候选
  与信号均有实质分歧"（真值缺席）→ 弃权归因。实测误归因场景该比值
  ≤ 0，干净往返 = 1.0，温和攻击 0.6~1.0；新参数
  `trace(match_score_floor=...)` 可调，None/0 关闭（回到 v0.14.1 行为）
- **embed 未注册 UID 告警**：注册库存在且 int UID 未注册时发
  UserWarning（str 别名自动注册不受影响），提示先
  `register(别名, uid=N)` 或显式忽略
- 覆盖三条 soft 子路径：自适应 / 冗余 layout（上限只计 layout 带）/
  default；对照场景（嵌入已注册 UID）双路径归因不受影响
- 新增回归测试 8 项（tests/test_v0142_issue23_fix.py）；核心 423
